...
class PixelReasonerTool(BaseTool):
    ...
    async def conduct_select_images_action_async(self, parameters, env):
        ...
        执行select_images动作 - 从查询图像或候选图像中选择特定的图像进行进一步分析
        """
        valid = False
        missing_parameters = []
        ...
        # 同时没有 query image 和 candidate images
        elif (env.get('query_image') is None) and (
            'candidate_images' not in env or env['candidate_images'] is None or len(env['candidate_images']) == 0
        ):
            observation = "No images available. Please ensure query_image or candidate_images are provided in extra_field."
        else:
            target_indices = parameters['target_images']

            # Check each index individually with specific error messages
            invalid_indices = []
            for idx in target_indices:
                if not isinstance(idx, int):
                    invalid_indices.append(f"Index {idx} is not an integer")
                elif idx == 0 and env.get('query_image') is None:
                    invalid_indices.append(f"Index {idx}: Query image does not exist. Please do not choose query image again and analyze based on query text.")
                elif idx != 0 and (env.get('candidate_images') is None or len(env['candidate_images']) == 0):
                    invalid_indices.append(f"Index {idx}: Candidate images do not exist. Please do not choose candidate images again and analyze based on candidates' text.")
                elif idx != 0 and not (1 <= idx <= len(env['candidate_images'])):
                    num_candidates = len(env['candidate_images']) if env.get('candidate_images') is not None else 0
                    invalid_indices.append(
                        f"Index {idx}: Invalid target_images indices. Each index should be an integer equal to 0 for the query image "
                        f"or between 1 and the number of candidate images ({num_candidates}) for candidate images."
                    )

            if invalid_indices:
                observation = "Invalid indices found: " + "; ".join(invalid_indices)
            else:
                try:
                    target_image_sources = []
                    for image in target_indices:
                        if image == 0:
                            target_image_sources.append(env['query_image'])
                        else:
                            target_image_sources.append(env['candidate_images'][image - 1])
                    ...
                    observation = {
                        'obs': (
                            "Here are the selected images for detailed analysis. "
                            f"(Image Size: {image_width}x{image_height}, Numbered 1 to {num_images}):"
                            + "<image>" * len(selected_images)
                        ),
    ...
    def load_env(self, trajectory_id):
        ...
        if env is None:
            env = {
                ...
                "images": [],
                "candidate_images": [],
                "query_image": None,  # 新增：单独保存 query image
                "temporary_images": [],
    ...
    async def conduct_zoom_in_action_async(self, parameters, env):
        ...
        支持对 query image（target_image=0）或 candidate image（1..N）进行裁剪
        """
        valid = False
        missing_parameters = []
        ...
        if missing_parameters:
            ...
            env.get('candidate_images') is None or len(env['candidate_images']) == 0
        ):
            observation = "No images available. Please ensure query_image or candidate_images are provided in extra_field."
        else:
            ti = parameters['target_image']
            
            # Check if target_image index is valid with specific error messages
            if not isinstance(ti, int):
                observation = "Invalid target_image format. It should be an integer."
            elif ti == 0 and env.get('query_image') is None:
                observation = "Query image does not exist. Please do not choose query image again and analyze based on query text."
            elif ti != 0 and (env.get('candidate_images') is None or len(env['candidate_images']) == 0):
                observation = "Candidate images do not exist. Please do not choose candidate images again and analyze based on candidates' text."
            elif ti != 0 and not (1 <= ti <= len(env['candidate_images'])):
                num_candidates = len(env['candidate_images']) if env.get('candidate_images') is not None else 0
                observation = (
                    "Invalid target_images indices. Each index should be an integer equal to 0 for the query image "
                    f"or between 1 and the number of candidate images ({num_candidates}) for candidate images."
                )
            else:
                try:
                    if ti == 0:
                        img_to_crop = env['query_image']
                    else:
                        img_to_crop = env['candidate_images'][ti - 1]
    ...
    async def _conduct_action_async(self, trajectory_id: str, action: str, extra_field: Dict[str, Any]):
        ...
        # 初始化 query_image（如果提供）
        if env.get('query_image') is None and "query_image" in extra_field:
            query_image_path = extra_field["query_image"]
            if isinstance(query_image_path, (str, os.PathLike)) and query_image_path:
                env['query_image'] = Path(query_image_path)
            else:
                env['query_image'] = None

        # 初始化图像列表（如果提供）
        if not env['images'] and "images" in extra_field:
            env['images'] = [Path(x) for x in extra_field["images"]]

        # 初始化候选图像列表（如果提供）
        if not env['candidate_images'] and "candidate_images" in extra_field:
