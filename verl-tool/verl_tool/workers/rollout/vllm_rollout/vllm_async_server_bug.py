...
class AsyncvLLMServer(VerlAsyncvLLMServer.__ray_actor_class__):
    async def init_engine(self):
        ...
        # **修复：在重新赋值 config 之前获取 val_only**
        val_only = config.trainer.get("val_only", False) if hasattr(config, "trainer") else False
        ...
        if val_only:
            logger.info("Val-only mode: Initializing vLLM for pure generation (no log_probs/ref).")
