...
def fsdp2_load_full_state_dict(model, full_state, device_mesh=None, cpu_offload=None):
    """
    Load full state dict into FSDP2 model with CPU-based broadcasting to avoid GPU 0 OOM.
    
    Args:
        model (`torch.nn.Module`): The sharded FSDP2 model.
        full_state (`dict`): The full state dict (loaded on rank 0, will be moved to CPU).
        device_mesh: FSDP2 device mesh.
        cpu_offload: Whether to offload to CPU after loading.
        
    Hack Explanation:
    - Rank 0: Move full_state to CPU (avoids GPU allocation).
    - For each param/buffer: Broadcast shape/dtype from rank 0, create CPU recv tensor, broadcast data from CPU rank 0.
    - Shard params (assume FSDP DP along dim=0 or dim=1), replicate buffers.
    - Copy shard/full to local GPU param/buffer.
    - Post-load: Apply cpu_offload if enabled.
    """
    import logging
    ...
    # Step 1: Initialize model on device (sharded, empty)
    if rank == 0:
        model = model.to(device=get_device_id(), non_blocking=True)
    else:
        model = model.to_empty(device=get_device_id())

    # Step 2: On rank 0, move full_state to CPU
    if rank == 0:
        logger.info("Moving full state_dict to CPU for broadcasting (to avoid GPU 0 OOM).")
        cpu_full_state = {k: v.cpu() if isinstance(v, torch.Tensor) else v for k, v in full_state.items()}
    else:
        cpu_full_state = None

    # Step 3: Get local model state (references to params/buffers)
    model_state = model.state_dict()  # Use standard state_dict for FSDP

    # Step 4: Process each key (params and buffers)
    for key in model_state.keys():
        try:
            local_tensor = model_state[key]
            if not isinstance(local_tensor, torch.Tensor):
                # Non-tensor (rare), copy from rank 0
                if rank == 0 and key in cpu_full_state:
                    model_state[key] = cpu_full_state[key]
                continue

            if rank == 0 and key in cpu_full_state:
                full_tensor_cpu = cpu_full_state[key]
                if not isinstance(full_tensor_cpu, torch.Tensor):
                    continue
                full_shape = full_tensor_cpu.shape
                full_dtype = full_tensor_cpu.dtype.name  # Send standard string like 'bfloat16'
            else:
                full_shape = None
                full_dtype = None

            # Broadcast shape and dtype from rank 0 (list of objects: shape tuple, dtype str)
            broadcast_list = [full_shape, full_dtype] if rank == 0 else [None, None]
            dist.broadcast_object_list(broadcast_list, src=0)
            full_shape = broadcast_list[0]
            full_dtype_str = broadcast_list[1]

            if full_shape is None or full_dtype_str is None:
                logger.warning(f"Rank {rank}: Skipping key {key} (no full shape/dtype from rank 0)")
                continue

            # Parse dtype safely
            try:
                full_dtype = torch.dtype(getattr(torch, full_dtype_str))
            except AttributeError:
                # Fallback to local dtype if parsing fails
                full_dtype = local_tensor.dtype
                logger.warning(f"Rank {rank}: Failed to parse dtype '{full_dtype_str}' for {key}, using local {full_dtype}")

            # All ranks: Create CPU recv tensor for full param/buffer
            full_recv_cpu = torch.empty(full_shape, dtype=full_dtype, device='cpu')

            # Rank 0: Fill with data before broadcast
            if rank == 0:
                full_recv_cpu.copy_(full_tensor_cpu)

            # Broadcast full tensor from CPU rank 0 to all ranks' CPU
            dist.broadcast(full_recv_cpu, src=0)

            # Now, determine if sharded (param) or replicated (buffer)
            local_shape = local_tensor.shape
            is_sharded = False
            shard_dim = 0  # Default FSDP sharding dim

            # Check for sharding along dim=0 (e.g., embeddings, biases) or dim=1 (e.g., linear weights)
            if len(full_shape) > 0:
                if full_shape[0] == local_shape[0] * world_size and len(local_shape) == len(full_shape):
                    is_sharded = True
                    shard_dim = 0
                elif len(full_shape) > 1 and full_shape[1] == local_shape[1] * world_size and len(local_shape) == len(full_shape):
                    is_sharded = True
                    shard_dim = 1
            ...
            if is_sharded:
                ...
                # Slice along shard_dim
                if shard_dim == 0:
                    shard_cpu = full_recv_cpu[offset:offset + local_size].clone()
                else:  # shard_dim == 1, slice columns
                    shard_cpu = full_recv_cpu[:, offset:offset + local_size].clone()
                local_tensor.copy_(shard_cpu.to(local_tensor.device))
                del shard_cpu
            else:
                # Replicated buffer: copy full to local
                assert full_shape == local_shape, f"Rank {rank}: Shape mismatch for {key}: full {full_shape}, local {local_shape}"
                local_tensor.copy_(full_recv_cpu.to(local_tensor.device))

            del full_recv_cpu  # Release CPU memory immediately
        except Exception as e:
            logger.error(f"Rank {rank}: Error processing key {key}: {e}. Skipping.")
            continue

    # Step 5: No need for model.load_state_dict() since we copied in-place to params/buffers

    # Step 6: Apply cpu_offload if enabled (post-load)
    if cpu_offload:
        logger.info("Applying CPU offload after loading.")
        model.to("cpu", non_blocking=True)
        # Move buffers back to GPU if needed (offload policy may handle)
        for buf in model.buffers():
    ...
    logger.info(f"CPU-based broadcast complete on rank {rank}. Model loaded without GPU 0 peak.")
