...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        import torch
        from torch.distributed.fsdp.api import FullStateDictConfig  # already imported at file top but safe to reference
        ...
        def __collect_lora_params() -> OrderedDict:
            ...
            if fsdp_version(self.module) > 0:
                if self.layered_summon:
                    ...
                    # use summon_full_params to safely gather LoRA params
                    try:
                        # prefer offloading to CPU if supported
                        ...
                        # older torch: no offload_to_cpu kwarg
                        with FSDP.summon_full_params(self.module, writeback=False):
                            if self.base_sync_done:
                                lora_params = get_peft_model_state_dict(peft_model)
                                lora_params = {
        ...
        self.timing = {}
        with simple_timer("reshard", self.timing):
            # try to free gpu caches before heavy ops
            get_torch_device().empty_cache()
            ...
            # collect params either from LoRA path or full model path but ensure tensors are on CPU
            if hasattr(peft_model, "peft_config"):
                peft_config = peft_model.peft_config.get("default", None)
                params = __collect_lora_params()
            else:
                # --- safer, multi-strategy approach to obtain full state dict on CPU ---
                params = None
                # 1) Prefer summon_full_params with offload_to_cpu=True (gathers to CPU)
                try:
                    with FSDP.summon_full_params(self.module, writeback=False, offload_to_cpu=True):
                        raw_sd = self.module.state_dict()
                        params = {k: v.detach().cpu() if isinstance(v, torch.Tensor) else v for k, v in raw_sd.items()}
                    get_torch_device().empty_cache()
                    logger.debug("Collected state_dict via summon_full_params(offload_to_cpu=True)")
                except TypeError:
                    # older torch versions may not accept offload_to_cpu kwarg
                    try:
                        with FSDP.summon_full_params(self.module, writeback=False):
                            raw_sd = self.module.state_dict()
                            params = {k: v.detach().cpu() if isinstance(v, torch.Tensor) else v for k, v in raw_sd.items()}
                        get_torch_device().empty_cache()
                        logger.debug("Collected state_dict via summon_full_params() (no offload_to_cpu)")
                    except Exception as e:
                        logger.warning("summon_full_params failed: %s. Try set_state_dict_type fallback.", e)
                        # 3) Fallback: temporarily set FULL_STATE_DICT with offload_to_cpu (if supported)
                        prev_state = None
                        try:
                            try:
                                # attempt to set to full state dict offloading to CPU
                                FSDP.set_state_dict_type(
                                    self.module,
                                ...
                                # FullStateDictConfig(offload_to_cpu=True) may not be supported on older torch
                                FSDP.set_state_dict_type(
                                    self.module,
                                    state_dict_type=StateDictType.FULL_STATE_DICT,
                            ...
                            logger.error("Fallback state_dict collection also failed: %s", e2)
                            raise
                        finally:
                            # try to restore to sharded state dict config to avoid side effects
                            try:
                                FSDP.set_state_dict_type(
                                    self.module,
                                    state_dict_type=StateDictType.SHARDED_STATE_DICT,
                                ...
                                # Not critical if restore fails; log and continue
                                logger.debug("Failed to restore SHARDED_STATE_DICT after fallback.")
                        get_torch_device().empty_cache()
    
            # convert keys as previously
            params = convert_weight_keys(params, getattr(self.module, "_fsdp_wrapped_module", self.module))
            log_gpu_memory_usage("After state_dict() in sharding manager memory", logger=logger)
    
            # wake up vllm engine, optionally freeing its cache first
            if self.rollout_config.free_cache_engine:
                if "tags" in inspect.signature(self.inference_engine.wake_up).parameters:
                    self.inference_engine.wake_up(tags=["weights"])
                else:
            ...
            # update model params in vLLM
            self.update_params(params, peft_config=peft_config)
            log_gpu_memory_usage("After sync model weights in sharding manager", logger=logger)

            # delete CPU params dict to release host memory; ensure GPU caches freed
            try:
                del params
            except Exception:
                pass
