...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        with simple_timer("reshard", self.timing):
            ...
            if hasattr(peft_model, "peft_config"):
                ...
                # ========== 关键修改：使用CPU offload收集参数 ==========
                try:
                    # 方法1: 优先使用 summon_full_params 的 offload_to_cpu 参数（PyTorch >= 2.0）
                    logger.info("Collecting state_dict with CPU offload using summon_full_params")
                    with FSDP.summon_full_params(self.module, writeback=False, offload_to_cpu=True):
                        ...
                        for name, param in self.module.state_dict().items():
                            ...
                            if isinstance(param, torch.Tensor):
                                ...
                                params[name] = param
                    logger.info(f"Successfully collected {len(params)} parameters on CPU")
                except TypeError as e:
                    # 方法2: 如果不支持 offload_to_cpu，使用传统的 summon_full_params 然后手动移到CPU
                    logger.warning(f"offload_to_cpu not supported ({e}), using summon_full_params without offload")
                    with FSDP.summon_full_params(self.module, writeback=False):
                        params = {}
                        for name, param in self.module.state_dict().items():
                            if isinstance(param, torch.Tensor):
                                ...
                                params[name] = param
                    logger.info(f"Collected {len(params)} parameters and moved to CPU")
                except Exception as e:
                    # 方法3: 降级方案 - 使用 FullStateDictConfig
                    logger.warning(f"summon_full_params failed ({e}), using FullStateDictConfig")
                    ...
                    with FSDP.state_dict_type(
                        ...
                        for name, param in self.module.state_dict().items():
                            if isinstance(param, torch.Tensor):
                                ...
                                params[name] = param
                    logger.info(f"Collected {len(params)} parameters using FullStateDictConfig")
            ...
            # update model params
            self.update_params(params, peft_config=peft_config, batch_size=15)
    ...
    def update_params(self, updated_params, peft_config=None, batch_size=20):
        """Update model parameters in the vLLM inference engine with batched transfer.
        ...
        device placement and memory management. Uses batched transfer to reduce
        ...
        Args:
            ...
            batch_size (int): Number of parameters to transfer per batch. Default: 20.
                            Smaller values use less memory but transfer slower.
        """
        ...
        if peft_config:
            if self.base_sync_done:
                ...
                logger.info(f"vLLM load LoRA weights, loaded_params: {len(updated_params)}")
                return
            else:
        ...
        # 统计参数信息
        total_params = len(updated_params)
        total_size_mb = sum(
        ...
        logger.info(f"Starting batched parameter transfer: {total_params} params, {total_size_mb:.2f} MB total")
        logger.info(f"Using batch_size={batch_size} for memory-efficient transfer")
        
        def batched_param_generator(params_dict, batch_size):
            """
            分批生成参数，降低GPU内存峰值。
            
            每批参数会先从CPU传输到GPU，然后立即被vLLM消费，
            之后GPU内存可以被释放，从而避免一次性加载所有参数到GPU。
            
            Args:
                params_dict (dict): 参数字典
                batch_size (int): 每批参数数量
                
            Yields:
                tuple: (param_name, param_tensor_on_gpu)
            """
            ...
            for batch_idx in range(num_batches):
                ...
                batch_size_mb = sum(
                    ...
                    for _, param in batch
                )
                
                logger.info(
                    f"Transferring batch {batch_idx + 1}/{num_batches}: "
                    f"{len(batch)} params, {batch_size_mb:.2f} MB "
                    f"({batch_start + 1}-{batch_end}/{len(items)})"
                )
                
                # 逐个处理参数，先移到GPU，然后yield
                for name, param in batch:
                    if isinstance(param, DTensor):
                        # DTensor需要先full_tensor，然后移到GPU
                        param_on_gpu = param.to(device, non_blocking=True).full_tensor()
                    elif isinstance(param, torch.Tensor):
                        # 普通Tensor，检查是否在CPU，如果在CPU则移到GPU
                        if param.device.type == 'cpu':
                            param_on_gpu = param.to(device, non_blocking=True)
                        else:
                            param_on_gpu = param
                    else:
                        # 非Tensor类型（如buffer），直接使用
                        param_on_gpu = param
                    
                    yield (name, param_on_gpu)
                
                # 每批传输后同步CUDA流并清理缓存
                torch.cuda.synchronize()
                
                # 适度清理GPU缓存（不要每批都清理，会影响性能）
                if (batch_idx + 1) % 5 == 0 or batch_idx == num_batches - 1:
                    get_torch_device().empty_cache()
                    logger.debug(f"Cleared GPU cache after batch {batch_idx + 1}")
        ...
        try:
            ...
            loaded_params = model.load_weights(
                batched_param_generator(updated_params, batch_size)
            )
            ...
            logger.info(
                f"vLLM load weights completed: "
                f"loaded_params={len(loaded_params) if loaded_params else -1}, "
                f"time={elapsed:.2f}s, "
                f"speed={total_size_mb/elapsed:.2f} MB/s"
            )
            
        except Exception as e:
            logger.error(f"Failed to load weights to vLLM: {e}")
            logger.error(f"Error occurred during batched transfer, trying fallback method...")
            
            # 降级方案：使用原始方法（不分批）
            logger.warning("Using fallback: loading all parameters at once (may cause OOM)")
            loaded_params = model.load_weights(
                (
            ...
            logger.info(f"Fallback completed, loaded_params: {len(loaded_params) if loaded_params else -1}")

        self.base_sync_done = True
        
        # 最后清理一次GPU缓存
        get_torch_device().empty_cache()
        log_gpu_memory_usage("After update_params completed", logger=logger)