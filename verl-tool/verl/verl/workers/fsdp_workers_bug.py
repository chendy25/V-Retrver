...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            # **val_only 模式：跳过 FSDP actor，只初始化 vLLM rollout**
            if self.config.trainer.val_only and self.config.trainer.total_training_steps == 0:
                logger.info("Val-only mode: Skipping FSDP actor loading, but initializing vLLM rollout.")

                # 跳过 FSDP actor/optimizer
                self.actor_module_fsdp = None
                self.actor_optimizer = None
                self.actor_lr_scheduler = None
                self.actor_model_config = None
            
                # **仅初始化 vLLM rollout（如果是 rollout 角色）**
                if self._is_rollout:
        ...
        # **actor 后续处理（仅训练模式）**
        if self._is_actor:
            if not (self.config.trainer.val_only and self.config.trainer.total_training_steps == 0):
        ...
        # **rollout 后续处理（仅训练模式，val_only 已在上面初始化）**
        if self._is_rollout:
            if not (self.config.trainer.val_only and self.config.trainer.total_training_steps == 0):
                self.rollout, self.rollout_sharding_manager = self._build_rollout(
                    trust_remote_code=self.config.model.get("trust_remote_code", False)
        ...
        # **ref 初始化（添加 val_only 跳过）**
        if self._is_ref:
            if self.config.trainer.val_only and self.config.trainer.total_training_steps == 0:
                logger.info("Val-only mode: Skipping ref model init.")
        ...
        # **checkpoint manager（仅非 val_only）**
        if self._is_actor:
            if not (self.config.trainer.val_only and self.config.trainer.total_training_steps == 0):
