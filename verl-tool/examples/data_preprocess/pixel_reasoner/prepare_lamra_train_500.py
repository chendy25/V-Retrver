...
system_prompt_original = """You are a helpful assistant for multimodal retrieval tasks.
...
system_prompt ="""You are a helpful assistant for multimodal retrieval tasks. Your goal is to evaluate a query (which can be text, image, or both) against multiple candidate items (which can be text, image, or both) and select the best matching candidate.
...
guideline_original = """Guidelines: You are given a query (which can be text, image, or both) and multiple candidate items (which can be text, image, or both). Your task is to evaluate each candidate against the query and select the best match. Determine if it is beneficial to employ the given visual operations (tools). You can use the following tools:
...
guideline = """Guidelines:Always begin by analyzing the query and all candidates in detail, covering both text and image components.
...
def process_lamra_data(lamra_file_path: str, output_dir: str, filter_len=None):
    ...
    with open(output_dir / "lamra_500.json", "w", encoding="utf-8") as f:
    ...
    # 保存数据
    train_dataset.to_parquet(output_dir / "lamra_train_500.parquet")
    val_dataset.to_parquet(output_dir / "lamra_val_500.parquet")

    print(f"Saved training data to {output_dir / 'lamra_train_500.parquet'}")
    print(f"Saved validation data to {output_dir / 'lamra_val_500.parquet'}")
...
def main(
    lamra_file_path: str = '/remote-home1/hxzhuang/cdy/LamRA/data/rerank_messages_500_quick.json',
